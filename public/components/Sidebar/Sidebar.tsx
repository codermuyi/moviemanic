...
const Sidebar = ({ iconFill, isOpen, toggle }: SidebarProps) => {
  ...
  const movieGenreLinks = movieGenres?.map((genre: Genre) => (
    <NavLink key={genre.id} href={routesV1.MOVIE_GENRE(genre.id)}>
      <p>{genre.name}</p>
    </NavLink>
  ))

  const tvGenreLinks = tvGenres?.map((genre: Genre) => (
    <NavLink key={genre.id} href={routesV1.TV_GENRE(genre.id)}>
      <p>{genre.name}</p>
    </NavLink>
  ))
